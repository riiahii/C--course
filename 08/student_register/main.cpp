...
void saveDataToFile(std::string const &filename, std::map<std::string, Student*> const &users){
...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            std::string student_num = parts[1];
            ...
            student_numbers.at(student_num)->phone_number = new_number;
            saveDataToFile(file_name, user_ids);
            std::cout << std::endl;
